...
function begin(details){
	...
	if (details.totalTests > 0) {
		console.log(`ESLint Rule (arrow-require-this) Test Suite (${details.totalTests})`);
		console.log("");
	}
	else {
		console.log(`ESLint Rule (arrow-require-this) Test Suite: empty!`);
		process.exit(1);
	}
}
...
function prettyPrint(v) {
	if (Array.isArray(v)) {
		...
		return JSON.stringify(v,function json(k,v){
			if (v === undefined) {
				return null;
			}
			return v;
